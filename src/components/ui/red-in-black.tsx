...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const compile = (type: number, src: string) => {
      const s = currentGl.createShader(type)!
      currentGl.shaderSource(s, src)
      currentGl.compileShader(s)
      return s
    }
    const program = currentGl.createProgram()!
    const vertexShader = compile(currentGl.VERTEX_SHADER, VERT)
    const fragmentShader = compile(currentGl.FRAGMENT_SHADER, FRAG)
    currentGl.attachShader(program, vertexShader)
    currentGl.attachShader(program, fragmentShader)
    currentGl.linkProgram(program)
    currentGl.deleteShader(vertexShader)
    currentGl.deleteShader(fragmentShader)
    currentGl.useProgram(program)

    const buf = currentGl.createBuffer()
    currentGl.bindBuffer(currentGl.ARRAY_BUFFER, buf)
    currentGl.bufferData(
      currentGl.ARRAY_BUFFER,
      new Float32Array([-1, -1, 3, -1, -1, 3]),
      currentGl.STATIC_DRAW,
    )
    const loc = currentGl.getAttribLocation(program, "a_position")
    currentGl.enableVertexAttribArray(loc)
    currentGl.vertexAttribPointer(loc, 2, currentGl.FLOAT, false, 0, 0)

    const uni = {
      colors: currentGl.getUniformLocation(program, "u_colors"),
      scene: currentGl.getUniformLocation(program, "u_scene"),
      shape: currentGl.getUniformLocation(program, "u_shape"),
      surface: currentGl.getUniformLocation(program, "u_surface"),
      finish: currentGl.getUniformLocation(program, "u_finish"),
      transform: currentGl.getUniformLocation(program, "u_transform"),
      space: currentGl.getUniformLocation(program, "u_space"),
      cursor: currentGl.getUniformLocation(program, "u_cursor"),
    }
    currentGl.uniform3fv(uni.colors, new Float32Array(UNIFORMS.colors.flat()))
    currentGl.uniform4f(
      uni.shape,
      UNIFORMS.scale,
      UNIFORMS.intensity,
    ...
    let bounds = currentCanvas.getBoundingClientRect()
    let raf = 0
    ...
    const resizeCanvas = () => {
      ...
      if (currentCanvas.width !== width || currentCanvas.height !== height) {
        ...
        currentGl.viewport(0, 0, width, height)
      }
    }
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      bounds = currentCanvas.getBoundingClientRect()
      updatePointerTarget()
    }
    ...
    return () => {
      ...
      currentGl.deleteBuffer(buf)
      currentGl.deleteProgram(program)
      const releaseTimer = window.setTimeout(() => {
        if (pendingContextReleases.get(currentCanvas) !== releaseTimer) return
        pendingContextReleases.delete(currentCanvas)
        currentGl.getExtension("WEBGL_lose_context")?.loseContext()
        currentCanvas.width = 1
        currentCanvas.height = 1
      }, 0)
      pendingContextReleases.set(currentCanvas, releaseTimer)
    }
  }, [])
